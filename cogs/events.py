...
class Events:
    ...
    # g e t   c l a p p e d   e r r o r s
    async def on_command_error(self, linu, err):
        if isinstance(err, errors.MissingRequiredArgument):
            embed = discord.Embed(  # look at that sexy embed
                title="Error :(",  # oof
                # totally user friendly
                description=f"You didnt give any args.... Usage (prefix) (command) (args)!",
        ...
        if isinstance(err, errors.BadArgument):
            ...
            embed = discord.Embed(  # look at that sexy embed
                # look at that sexy color
                domcolor=await linu.get_dominant_color(linu.author.avatar_url),
                title="Ratelimit",  # oof
                # mission failed
            ...
            # delete after 5 seconds so it dont flood chat
            await linu.send(embed=embed, delete_after=5)
    ...
    async def on_guild_join(self, guild):
        ...
        try:
            ...
            try:
                invite_chan = sorted([chan for chan in guild.channels if chan.permissions_for(
                    guild.me).create_instant_invite and isinstance(chan, discord.TextChannel)], key=lambda x: x.position)[0]
    ...
    async def on_guild_remove(self, guild):
        ...
        bots = set(bots)
        members = len(members) - len(bots)
        leave = discord.Embed(title="Removed from Guild",
    ...
    async def on_message_edit(self, before, after):
        ...
        # p r o c e s   t h o s e   c o m m a n d s
    ...
    async def on_ready(self):
        ...
        await self.bot.change_presence(activity=discord.Game(type=0, name=f"with {self.gettotalusers()} users  | linu help"), status=discord.Status.online)
        print(
            f'Servers: {len(self.bot.guilds)} | Users: {len(set(self.bot.get_all_members()))}')
        print('---------------')
